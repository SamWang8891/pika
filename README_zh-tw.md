...
5. 建構前端：
   ...
   npm run build
   ```
6. 將 `dist` 資料夾複製到 `docker/` 並將其命名為 `frontend`。
...
如果要修改後端，請按照以下步驟。否則，將 `backend` 資料夾複製到 `docker/` 並將其命名為 `backend`。
...
6. 在開發模式執行：
   ...
   python app.py
   ```
7. 在更改後端後，將 `backend` 資料夾複製到 `docker/` 並將其命名為 `backend`。
