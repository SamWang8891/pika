...
- 縮短網址也可以自定義。
- 不想要英文單字？另一顆按鈕可改用短隨機字串（全小寫，4 個字元起）產生短網址。
...
不想要字典單字嗎？主頁上的 **Shorten with random string** 會請伺服器改配一組不透明的英數字串。伺服器端的配發方式：

- 字串**全為小寫**（`a-z0-9`），從 **4 個字元**開始（約 170 萬種組合）——手動輸入時不用猜大小寫。
- 直接以 INSERT 佔用該字串——資料庫的 `UNIQUE` 約束就是碰撞檢查，因此兩個同時的請求絕不會拿到同一組字串。
- 連續 **碰撞 10 次**後長度加一，再重試。
- 隨機字串模式每次都產生新的字串：同一個網址縮兩次會得到兩條不同的短網址（與字典模式不同，字典模式會回傳既有的那條）。

透過 API 也可以——在 `create_record` 加上 `random_string: true`：
